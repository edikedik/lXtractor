...
def parse_var(inp: str):
    ...
    LOGGER.debug(
        f"Split input {inp} into Variables={variables},"
...
def init_var(var: str):
    ...
    def split_pos(_pos: str) -> t.Tuple[int, t.Optional[str]]:
        ...
        if len(_split) == 2:
            ...
            raise FormatError(
                f'Expected 1 element or 2 ":"-separated elements. ' f"Got {var}"
            )
        try:
            if _pos != "ALL":
                _pos = int(_pos)
        except ValueError:
            raise FormatError(f"Non-convertable position {_pos} in {var}")
        return _pos, atom
    ...
    if len(split) == 1:
        ...
        if var.isdigit():
            ...
            if not pos.isdigit():
                raise FormatError(f"Non-convertable position {pos} in {var}")
        ...
        LOGGER.debug(
            f"Received two positions {split} " f'to init the "distance" variable'
        )

        (pos1, atom1), (pos2, atom2) = map(split_pos, split)

        LOGGER.debug(
            f"Split the first position into {pos1}:{atom1}, "
        ...
        LOGGER.debug(
            f"Received four positions {split} " f'to init "dihedral"-type variable'
        )
        ...
        LOGGER.debug(
            f"Split each of the position into the following pos-atom pairs: "
